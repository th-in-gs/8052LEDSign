...
void serialPrintDec(const auto number, const bool wait = false);
...
void serialPrintDec(const auto number, const bool wait)
{
    decltype(number) toPrint = number % 10;
    decltype(number) remaining = number / (decltype(number))10;
    if(remaining) {
